...
def iview(): 
    ...
    headingLabel = Label(headingFrame1, text="Issued Books list", bg='white', fg='black', font=('Arial',17))
    ...
    Label(labelFrame, text="%-10s%-40s%-30s%-20s"%('BID','Title','Author','Issued to'),bg='white',fg='black').place(relx=0.07,rely=0.1)
    Label(labelFrame, text="----------------------------------------------------------------------------",bg='white',fg='black').place(relx=0.05,rely=0.2)
    getBooks = "select books.bid, books.title, books.author, books_issued.issuedto from books inner join books_issued on books.bid=books_issued.bid"
    try:
